/**
 * Deletes a comment for a course forum entry for the logged-in user
 */
...
export default eventHandler(async (event) => {
  ...
  const { error } = await (client.from("kommentar_kurs") as any)
    .delete()
    .eq("id", id)
    .eq("nutzerID", userId);

  if (error) {
    console.error("Kommentar-Kurs DELETE Error:", error);
    throw createError({
      statusCode: 500,
