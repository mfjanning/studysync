/**
 * Deletes a comment for a file for the logged-in user
 */
...
export default eventHandler(async (event) => {
  ...
  const { error } = await (client.from("kommentar_datei") as any)
    .delete()
    .eq("id", id)
    .eq("nutzerID", userId);

  if (error) {
    console.error("Kommentar-Datei DELETE Error:", error);
    throw createError({
      statusCode: 500,
