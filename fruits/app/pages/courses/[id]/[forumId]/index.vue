...
const loeschenBestaetigen = async () => {
  ...
  loeschFehler.value = ''
  try {
    await $fetch(`/api/kurse/foren/kommentare/${zuLoeschendenKommentarId.value}`, { method: 'DELETE' })
...
<template>
  <div class="min-h-screen bg-slate-50 dark:bg-gray-900 font-sans text-slate-800 dark:text-gray-100 pb-12 transition-colors duration-300">
    <div class="max-w-4xl mx-auto p-4 md:p-8">

      <div class="mb-6 pl-2">
        <NuxtLink
          :to="`/courses/${kursId}`"
          class="inline-flex items-center text-green-600 dark:text-green-400 hover:text-blue-700 dark:hover:text-blue-300 font-bold transition-colors"
        >
      ...
      <div class="bg-white dark:bg-gray-800 rounded-[2rem] shadow-xl shadow-green-900/5 dark:shadow-black/20 border border-slate-100 dark:border-gray-700 flex flex-col overflow-hidden transition-colors duration-300">

        <div class="p-5 bg-slate-50 dark:bg-gray-800/50 border-b border-slate-100 dark:border-gray-700">
          <h2 class="font-extrabold text-slate-700 dark:text-gray-200 uppercase tracking-widest text-sm">
            Kommentare
        ...
        <div class="p-5 space-y-4">
          ...
          <div
            ...
            :key="k.id"
            class="bg-slate-50 dark:bg-gray-900 rounded-2xl p-4 border border-slate-100 dark:border-gray-700 transition-colors duration-300"
          >
            <div class="flex items-center gap-3 mb-2">
              ...
              <span class="font-bold text-sm text-slate-800 dark:text-gray-200 flex-1">{{ k.profile?.name || 'Nutzer' }}</span>
              <span class="text-xs text-slate-400 dark:text-gray-500 font-medium">
                {{ new Date(k.created_at).toLocaleDateString('de-DE', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' }) }}
              </span>
              <button
                v-if="user?.id === k.nutzerID"
                @click="kommentarLoeschen(k.id)"
                :disabled="loeschtKommentarId === k.id"
                class="text-slate-300 dark:text-gray-600 hover:text-red-500 disabled:opacity-40 transition-colors text-xs font-bold leading-none"
            ...
            <p class="text-sm text-slate-700 dark:text-gray-300 leading-relaxed pl-11" style="overflow-wrap:anywhere;">{{ k.kommentar }}</p>
          </div>
        </div>
        ...
        <div class="p-5 bg-slate-50 dark:bg-gray-800/50 border-t border-slate-100 dark:border-gray-700">
          <div v-if="user">
            ...
            <textarea
              ...
              class="w-full p-3 bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-700 rounded-2xl text-sm text-slate-800 dark:text-gray-100 resize-none focus:outline-none focus:ring-2 focus:ring-green-500 transition"
              rows="4"
            ...
            <div class="flex justify-between items-center mt-2">
              <div />
              <p v-if="neuerKommentar.length > 0" class="text-xs font-medium" :class="neuerKommentar.length >= 900 ? 'text-orange-500' : 'text-slate-400 dark:text-gray-500'">
            ...
            <button
              ...
              class="mt-3 w-full bg-slate-800 dark:bg-slate-700 text-white font-bold py-3 rounded-full hover:bg-green-600 dark:hover:bg-green-500 disabled:opacity-50 transition-colors shadow-md text-sm"
            >
          ...
          <div v-else class="text-center py-4">
            ...
            <NuxtLink to="/login" class="inline-block bg-slate-800 dark:bg-slate-700 text-white font-bold py-2 px-6 rounded-full hover:bg-green-600 transition-colors text-sm">
              Anmelden
            </NuxtLink>
          </div>
        </div>
